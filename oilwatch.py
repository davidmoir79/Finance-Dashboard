...
def load_data():
    ...
    for enc in encodings:
        try:
            df = pd.read_csv(DATA_FILE, encoding=enc, sep=";", skipinitialspace=True)
            break
        except:
            pass
    ...
    df["Montly Sales"] = df["Montly Sales"].apply(clean_sales) if "Montly Sales" in df.columns else 0.0
...
with tab1:
    ...
    if st.session_state.df.empty:
        ...
        cur_start, cur_end = month_bounds(current_year, current_month)
        ...
        if company_choice == "Both Companies":
            cols = st.columns(2)
            with cols[0]:
...
with tab3:
    ...
    st.write(f"Database file: `{DATA_FILE}`")
    data_preview = money_frame(st.session_state.df)
