...
with tab1:
    ...
    if st.session_state.df.empty:
        ...
        m4.metric("Total Sales", fmt_rand(view_total["Monthly Sales"].sum()), "All time")
        ...
        show_grouped_bar_last_3_years(view_df, "Transformer Oil Lab")
...
with tab2:
    ...
    if st.session_state.df.empty:
        ...
        view_df2 = st.session_state.df[st.session_state.df["Company"].isin(TRANSFORMER_CODES)].copy()
        ...
        if not top_all.empty:
            ...
            ax.set_title("Top Customers - Transformer Oil Lab")
            ax.set_xlabel("Rand")
            ax.tick_params(axis="y", labelsize=8)
            ax.tick_params(axis="x", labelsize=8)
...
with tab3:
    ...
    st.dataframe(money_frame(filtered_data), use_container_width=True, hide_index=True)
